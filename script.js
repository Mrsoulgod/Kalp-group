...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const adjustLayoutForAdmin = (isAdminActive) => {
        ...
        if (portalContainer && portalCard) {
            if (isAdminActive) {
                ...
                portalContainer.style.width = '96%';
        ...
        try {
            ...
            inquiriesTableBody.innerHTML = inquiries.map(inq => `
                <tr>
                    ...
                    <td>${inq.name}</td>
                    <td>${inq.company}</td>
                    <td><a href="mailto:${inq.email}" style="color: var(--color-accent); text-decoration: underline;">${inq.email}</a></td>
                    <td><span class="product-card-badge" style="position: static; font-size: 0.75rem; background: var(--color-primary-light); color: white; display: inline-block;">${inq.productOrSubject}</span></td>
                    <td style="font-size: 0.85rem; max-width: 250px; white-space: normal; word-break: break-word;">${inq.message}</td>
                </tr>
            `).reverse().join('');
        } catch (e) {
    ...
    if (portalLoginForm) {
        portalLoginForm.addEventListener('submit', (e) => {
            ...
            if (currentRole === 'user') {
                ...
                if (usernameInput === 'arvind 001' && passwordInput === '001 Arvind') {
                    loginFormWrapper.style.display = 'none';
    ...
    if (adminDashboard) {
        ...
        const loadDashboardData = () => {
            ...
            actsContainer.innerHTML = acts.map(a => `
                <div style="font-size:0.8rem; border-bottom:1px solid #f1f5f9; padding:8px 0; display:flex; justify-content:space-between; gap:10px;">
                    <span style="color:#64748b; white-space:nowrap;">${a.time}</span>
                    <span style="color:#334155; text-align:right;">${a.text}</span>
                </div>
            `).join('');
        ...
        window.renderProducts = () => {
            ...
            selectModal.innerHTML = cats.map(c => `<option value="${c}">${c}</option>`).join('');

            const tbody = document.getElementById('productsTableBody');
            tbody.innerHTML = prods.map(p => `
                <tr>
                    <td><img src="${p.img || 'assets/images/logo.jpg'}" style="width:40px; height:40px; object-fit:cover; border-radius:4px; border:1px solid #cbd5e1;"></td>
                    <td style="font-weight:600;">${p.name}</td>
                    <td>${p.cas || 'N/A'}</td>
                    <td><span style="font-size:0.75rem; background:#cbd5e1; color:#1e293b; padding:2px 8px; border-radius:10px;">${p.category}</span></td>
                    <td>${p.moq || 'N/A'}</td>
                    <td><span style="font-size:0.75rem; background:#dcfce7; color:#166534; padding:2px 8px; border-radius:10px; font-weight:600;">${p.availability}</span></td>
                    <td>
                        <div style="display:flex; gap:6px;">
                            <button class="admin-btn admin-btn-secondary" onclick="editProduct('${p.id}')">Edit</button>
                            <button class="admin-btn admin-btn-secondary" onclick="duplicateProduct('${p.id}')" style="background:#f1f5f9; color:#0f766e;">Duplicate</button>
        ...
        window.renderCategories = () => {
            ...
            tbody.innerHTML = cats.map((c, i) => `
                <tr>
                    ...
                    <td>
                        <div style="display:flex; gap:6px;">
                            <button class="admin-btn admin-btn-danger" onclick="deleteCategory('${c}')">Delete</button>
                        </div>
                    </td>
                </tr>
            `).join('');
        ...
        window.renderCustomers = () => {
            ...
            tbody.innerHTML = custs.map(c => `
                <tr>
                    ...
                    <td>${c.person}</td>
                    <td>${c.email}<br><span style="color:#64748b; font-size:0.8rem;">${c.phone}</span></td>
                    <td>${c.gst || 'N/A'}</td>
                    <td>${c.country}</td>
                    <td><span style="font-size:0.75rem; background:#dcfce7; color:#166534; padding:2px 8px; border-radius:10px; font-weight:600;">${c.status}</span></td>
                    <td>
                        <button class="admin-btn admin-btn-secondary" onclick="viewCustomerDetails('${c.id}')">View History</button>
                    </td>
                </tr>
            `).join('');
        };
        ...
        window.renderInquiriesBoard = () => {
            ...
            tbody.innerHTML = enqs.map(e => `
                <tr>
                    <td style="font-weight:600; color:var(--color-primary);">${e.id || 'ENQ' + Math.floor(Math.random()*1000)}</td>
                    <td><strong>${e.company || 'N/A'}</strong><br><span style="color:#64748b; font-size:0.8rem;">${e.name}</span></td>
                    <td>${e.product}</td>
                    <td>${e.qty || 'N/A'}</td>
                    <td>${e.country || 'N/A'}</td>
                    <td><span style="font-size:0.75rem; background:${e.status === 'New' ? '#ffedd5' : '#e2e8f0'}; color:${e.status === 'New' ? '#ea580c' : '#475569'}; padding:2px 8px; border-radius:10px; font-weight:600;">${e.status}</span></td>
                    <td>
                        <div style="display:flex; gap:6px;">
                            <button class="admin-btn admin-btn-primary" onclick="openInquiryModal('${e.id}')">Process</button>
                        </div>
                    </td>
                </tr>
            `).join('');
        ...
        window.renderQuotes = () => {
            ...
            tbody.innerHTML = quotes.map(q => `
                <tr>
                    <td style="font-weight:600; color:var(--color-primary);">${q.id}</td>
                    <td><strong>${q.customer}</strong></td>
                    <td>${q.product} (${q.qty})</td>
                    <td style="font-weight:700; color:#166534;">$${(parseFloat(q.price) * parseFloat(q.qty) || 0).toLocaleString()}</td>
                    <td>${q.validUntil}</td>
                    <td><span style="font-size:0.75rem; background:${q.status === 'Accepted' ? '#dcfce7' : '#f1f5f9'}; color:${q.status === 'Accepted' ? '#166534' : '#334155'}; padding:2px 8px; border-radius:10px; font-weight:600;">${q.status}</span></td>
                    <td>
                        <div style="display:flex; gap:6px;">
                            <button class="admin-btn admin-btn-secondary" onclick="exportQuotePDF('${q.id}')">Export PDF</button>
                            <button class="admin-btn admin-btn-primary" onclick="simulateEmailQuote('${q.id}')" style="background:#3b82f6;">Email Quote</button>
                            <button class="admin-btn admin-btn-danger" onclick="deleteQuote('${q.id}')">Delete</button>
                        </div>
                    </td>
                </tr>
            `).join('');
        ...
        window.viewCustomerDetails = (id) => {
            ...
            body.innerHTML = `
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:10px; font-size:0.85rem; border-bottom:1px solid #cbd5e1; padding-bottom:15px;">
                    <div><strong>Contact Person:</strong> ${c.person}</div>
                    <div><strong>Industry Sector:</strong> ${c.industry}</div>
                    <div><strong>GST / Tax ID:</strong> ${c.gst || 'N/A'}</div>
                    <div><strong>Country Location:</strong> ${c.country}</div>
                ...
                <div>
                    ...
                    <div style="font-size:0.8rem; color:#475569;">
                        ${custQuotes.map(q => `<div>Quote ${q.id} - ${q.product} (${q.qty}) - Status: ${q.status}</div>`).join('') || 'No quotations issued.'}
                    </div>
                </div>
            `;
        ...
        window.openInquiryModal = (id) => {
            ...
            body.innerHTML = `
                <div style="font-size:0.9rem; display:flex; flex-direction:column; gap:8px;">
                    <div><strong>Inquiry ID:</strong> ${e.id}</div>
                    <div><strong>Company Name:</strong> ${e.company || 'N/A'}</div>
                    <div><strong>Contact Sender:</strong> ${e.name} (${e.email})</div>
                    <div><strong>Product Required:</strong> ${e.product}</div>
                    <div><strong>Quantity Required:</strong> ${e.qty || 'N/A'}</div>
                    <div><strong>Origin Country:</strong> ${e.country || 'N/A'}</div>
